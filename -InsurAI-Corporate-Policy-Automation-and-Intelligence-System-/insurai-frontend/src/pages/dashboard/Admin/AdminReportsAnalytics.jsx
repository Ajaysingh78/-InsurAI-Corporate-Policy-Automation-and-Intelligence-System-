...
export default function AdminReportsAnalytics() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      setLoading(true);
      try {
        const [usersRes, claimsRes, policiesRes, hrsRes, agentsRes] =
          await Promise.all([
            axios.get(
            ...
            axios.get(
              "https://insurai-backend-production.up.railway.app/admin/claims",
              { headers: { Authorization: `Bearer ${token}` } },
            ),
            axios.get(
              "https://insurai-backend-production.up.railway.app/admin/policies",
              { headers: { Authorization: `Bearer ${token}` } },
            ),
            ...
            axios.get(
              "https://insurai-backend-production.up.railway.app/agent",
              { headers: { Authorization: `Bearer ${token}` } },
            ),
          ]);
  ...
  const filteredClaims = useMemo(
    () => filterDataByDate(claims, "claims"),
    [claims, dateFilter],
  );
  const filteredUsers = useMemo(
    () => filterDataByDate(users, "users"),
    [users, dateFilter],
  );
  const filteredPolicies = useMemo(
    () => filterDataByDate(policies, "policies"),
    [policies, dateFilter],
  );
  ...
  const employeesWithClaims = users.filter(
    (user) => employeeClaimsMap[user.id] > 0,
  ).length;

  // Average claims per employee
  const averageClaimPerEmployee =
    totalEmployees > 0
      ? (filteredClaims.length / totalEmployees).toFixed(1)
      : 0;

  // Average amount per employee
  const averageAmountPerEmployee =
    totalEmployees > 0
      ? (
  ...
  const policyUsage = policies.map((policy) => {
    const policyClaims = filteredClaims.filter(
      (claim) =>
        claim.policyId === policy.policyId ||
        claim.policyName === policy.policyName,
    );
    const claimCount = policyClaims.length;
    const totalAmount = policyClaims.reduce(
      (sum, claim) => sum + (parseFloat(claim.amount) || 0),
      0,
    );
  ...
  const downloadFullPDFReport = () => {
    ...
    doc.text(
      `Date Filter: ${dateFilter === "all" ? "All Time" : dateFilter}`,
      105,
      34,
      { align: "center" },
    );
    ...
    doc.autoTable({
      ...
      body: [
        ...
        [
          "Total Claims",
          totalClaims,
          `₹${filteredClaims.reduce((sum, claim) => sum + (parseFloat(claim.amount) || 0), 0).toFixed(2)}`,
        ],
        [
          "Approved Claims",
          claimsByStatus.find((s) => s.name === "Approved")?.value || 0,
          "-",
        ],
        [
          "Pending Claims",
          claimsByStatus.find((s) => s.name === "Pending")?.value || 0,
          "-",
        ],
      ],
      theme: "grid",
    ...
    doc.autoTable({
      startY: startY + 5,
      head: [
        ["Employee", "Policy", "Amount (₹)", "Date", "Status", "Assigned HR"],
      ],
      body: filteredClaims
        .slice(0, 15)
        .map((claim) => [
    ...
    doc.autoTable({
      startY: startY + 5,
      head: [
        ["Policy Name", "Type", "Claims", "Total Amount (₹)", "Avg/Claim (₹)"],
      ],
    ...
    for (let i = 1; i <= pageCount; i++) {
      ...
      doc.text("Generated by Claims Management System", 105, 295, {
        align: "center",
      });
    }

    doc.save(
      `admin_comprehensive_report_${new Date().toISOString().split("T")[0]}.pdf`,
    );
  };
  ...
  return (
    ...
    >
          ...
          <div className="d-flex justify-content-between align-items-center pb-3 border-bottom">
            <div>
              ...
              <p
                ...
                className="mb-0 ms-5 ps-2"
              >
                Comprehensive insights across employees, HR, agents, policies,
                and claims
              </p>
            </div>
              ...
              <div className="row align-items-center">
                ...
                <div className="col-md-9">
                  ...
                  <div className="d-flex gap-2 flex-wrap align-items-center">
                    ...
                    <span
                      className="badge bg-danger px-3 py-2 shadow-sm"
                      style={{ borderRadius: "10px" }}
                    >
                    ...
                    <span
                      className="badge bg-dark px-3 py-2 shadow-sm"
                      style={{ borderRadius: "10px" }}
                    >
      ...
      <div className="row mb-4">
        <div className="col-12">
          <ul
            className="nav nav-pills nav-fill p-2 shadow-sm"
            style={{
            ...
            >
              <div className="card-body p-4">
                <div className="d-flex justify-content-between align-items-start">
                  <div>
                    <div
                      className="text-uppercase mb-1 small fw-bold"
                      style={{
                        color: COLORS.PRIMARY,
                        letterSpacing: "0.5px",
                        fontSize: "0.7rem",
            ...
            >
              <div className="card-body p-4">
                <div className="d-flex justify-content-between align-items-start">
                  ...
                  >
                    <i
                      className="bi bi-person-badge-fill fs-3"
            ...
            >
              <div className="card-body p-4">
                <div className="d-flex justify-content-between align-items-start">
                  ...
                  >
                    <i
                      className="bi bi-file-earmark-text-fill fs-3"
                ...
                <div className="d-flex justify-content-between align-items-start">
                  <div>
                    ...
                    <div
                      ...
                      style={{ color: COLORS.DARK, fontSize: "1.2rem" }}
                    >
                      {formatCurrency(
                        filteredClaims.reduce(
                          (sum, claim) => sum + (parseFloat(claim.amount) || 0),
              ...
              <div className="card-body">
                {claimsByStatus.length > 0 ? (
                  ...
                  <div className="text-center py-5">
                    <i
                      className="bi bi-pie-chart display-4 mb-3"
            ...
            >
              ...
              <div className="card-body">
                {policyUsage.length > 0 ? (
                  ...
                  <div className="text-center py-5">
                    <i
                      className="bi bi-bar-chart display-4 mb-3"
                      style={{ color: `${COLORS.ACCENT_2}30` }}
                    ></i>
                    ...
                    <div className="row text-center g-3">
                      ...
                      <div className="col-6">
                        <div
                          ...
                          style={{
                            borderRadius: "12px",
                            backgroundColor: `${COLORS.CONTRAST}08`,
                          }}
                        >
                          <h3
                            className="mb-1 fw-bold"
                    ...
                    <div className="table-responsive">
                      <table className="table table-hover align-middle mb-0">
                        ...
                        >
                          <tr>
                            <th
                              className="border-0 py-3 px-4 fw-bold"
                              style={{ color: COLORS.DARK }}
                            >
                              Name
                            </th>
                        ...
                        <tbody>
                          {users.slice(0, 10).map((user) => (
                            <tr key={user.id || user.employeeId}>
                              ...
                              <td>
                                <span
                                  className={`badge px-3 py-2 shadow-sm ${user.status === "Active" || !user.status ? "bg-success" : "bg-warning"}`}
                                  style={{ borderRadius: "8px" }}
                                >
                                  {user.status || "Active"}
                                </span>
                      ...
                      <tbody>
                        {hrs.map((hr) => {
                          const hrClaims = filteredClaims.filter(
                            (claim) =>
                              Number(claim.assignedHrId) === Number(hr.id),
                          );

                          const approved = hrClaims.filter(
                            (c) => c.status === "Approved",
                          ).length;
                          const rejected = hrClaims.filter(
                            (c) => c.status === "Rejected",
                          ).length;
                          const pending = hrClaims.filter(
                            (c) => c.status === "Pending",
                          ).length;
                          const total = hrClaims.length;
                          const approvalRate =
                          ...
                          return (
                            <tr key={hr.hrId || hr.id}>
                              ...
                              <td>
                                <span
                                  className="badge bg-warning px-3 py-2 shadow-sm"
                                  style={{ borderRadius: "8px" }}
                                >
                                  {pending}
                                </span>
            ...
            <div className="col-12">
              ...
              >
                <div className="card-body text-center py-5">
                  ...
                  >
                    <i
                      className="bi bi-person-badge display-1"
                      style={{ color: COLORS.CONTRAST }}
                    ></i>
                  </div>
                  <h4 className="fw-bold mb-2" style={{ color: COLORS.DARK }}>
              ...
              <div className="col-md-6 mb-4">
                ...
                >
                  <div className="card-body text-center py-5">
                    ...
                    <h5 className="mb-2" style={{ color: COLORS.DARK }}>
                      Total Agents
                    </h5>
                    <p style={{ color: COLORS.TEXT_MUTED }}>
                      Active insurance agents in the system
                    </p>
                  </div>
                </div>
              </div>
                ...
                >
                  ...
                  <div className="card-body">
                    <div className="table-responsive">
                      <table className="table table-hover align-middle mb-0">
                        <thead
                          style={{ backgroundColor: `${COLORS.CONTRAST}08` }}
                        >
                          <tr>
                            <th
                              className="border-0 py-3 fw-bold"
            ...
            <div className="col-12">
              ...
              >
                <div className="card-body text-center py-5">
                  ...
                  >
                    <i
                      className="bi bi-file-earmark-text display-1"
                      style={{ color: COLORS.WARNING }}
                    ></i>
                  </div>
                  <h4 className="fw-bold mb-2" style={{ color: COLORS.DARK }}>
                      ...
                      <tbody>
                        {policyUsage.map((policy) => (
                          <tr key={policy.policyId || policy.id}>
                            <td className="px-4" style={{ color: COLORS.DARK }}>
                              {policy.policyName}
                            </td>
                            <td>
                              <span
                                className="badge bg-secondary px-3 py-2 shadow-sm"
                                style={{ borderRadius: "8px" }}
                              >
                                {policy.policyType}
                              </span>
                            ...
                            <td
                              style={{ color: COLORS.DARK, fontWeight: "600" }}
                            >
                              {formatCurrency(policy.avgPerClaim)}
                            </td>
                          </tr>
                        ))}
                      </tbody>
                        ...
                        <tbody>
                          {filteredClaims.slice(0, 10).map((claim) => {
                            const employee = users.find(
                              (u) => u.id === claim.employeeId,
                            );
                            const hr = hrs.find(
                              (h) => h.id === claim.assignedHrId,
                            );
