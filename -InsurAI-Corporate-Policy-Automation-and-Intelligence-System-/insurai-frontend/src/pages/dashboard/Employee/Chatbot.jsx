...
const Chatbot = ({
  employeeData = { name: "Employee", claims: [], policies: [] },
}) => {
  const [isOpen, setIsOpen] = useState(false);
  const [messages, setMessages] = useState([
    {
      text: "Hello! I'm your InsurAI assistant. Ask me anything about your claims or policies. 🤖",
      sender: "bot",
    },
  ]);
  ...
  const handleSendMessage = async (e) => {
    ...
    if (!token) {
      setMessages((prev) => [
        ...prev,
        { text: "⚠️ Please log in to use InsurAI.", sender: "bot" },
      ]);
      setLoading(false);
      return;
    }
    ...
    try {
      ...
      const reply =
        response?.data?.response || "🤖 Sorry, I didn't catch that.";
      ...
      if (error.response?.status === 401)
        msg = "⚠️ Authentication failed. Please log in.";
      else if (error.response?.status === 403)
        msg = "⚠️ Access forbidden. Please check your permissions.";
  ...
  const styles = {
    ...
    chatWindow: {
      ...
      borderRadius: "24px",
      boxShadow:
        "0 25px 80px rgba(0, 0, 0, 0.5), 0 0 60px rgba(56, 189, 248, 0.15)",
      display: "flex",
